...
<div class="mb-8 flex items-center gap-4">
    ...
    <div>
        <h1 class="text-4xl font-black uppercase tracking-tighter">Edit Expertise</h1>
    </div>
</div>

<form action="{{ route('admin.expertises.update', $expertise) }}" method="POST" enctype="multipart/form-data" class="space-y-8 bg-white border-2 border-black p-6 md:p-8 rounded-lg shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
    @csrf
    @method('PUT')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $expertise->name) }}" class="w-full border-2 border-black p-3 focus:ring-0 focus:outline-none focus:border-primary transition" required>
        ...
        <div>
            ...
            <input type="text" name="category" value="{{ old('category', $expertise->category) }}" class="w-full border-2 border-black p-3 focus:ring-0 focus:outline-none focus:border-primary transition" required>
    ...
    <div>
        ...
        <textarea name="description" rows="4" class="w-full border-2 border-black p-3 focus:ring-0 focus:outline-none focus:border-primary transition">{{ old('description', $expertise->description) }}</textarea>
    ...
    <div>
        ...
        @if($expertise->logo_path)
            <div class="mt-2 flex items-center gap-4">
                <img src="{{ asset($expertise->logo_path) }}" class="h-12 border-2 border-black p-1">

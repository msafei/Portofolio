...
<div class="mb-8 flex items-center gap-4">
    ...
    <div>
        <h1 class="text-4xl font-black uppercase tracking-tighter">Add Experience</h1>
    </div>
</div>

<form action="{{ route('admin.experiences.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8 bg-white border-2 border-black p-6 md:p-8 rounded-lg shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
    @csrf

    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
            <label class="block text-sm font-black uppercase tracking-widest mb-2">Company Name</label>
            <input type="text" name="company" value="{{ old('company') }}" class="w-full border-2 border-black p-3 focus:ring-0 focus:outline-none focus:border-primary transition" required>
        ...
        <div>
            ...
            <input type="text" name="role" value="{{ old('role') }}" class="w-full border-2 border-black p-3 focus:ring-0 focus:outline-none focus:border-primary transition" required>
        ...
        <div class="col-span-1 md:col-span-2">
            ...
            <div class="flex flex-col md:flex-row items-center gap-4">
                <input type="month" id="start_month" class="w-full md:w-1/2 border-2 border-black p-3 focus:ring-0 focus:outline-none focus:border-primary transition" required>
                <span id="date_separator" class="font-black text-xl hidden md:block">-</span>
                <input type="month" id="end_month" class="w-full md:w-1/2 border-2 border-black p-3 focus:ring-0 focus:outline-none focus:border-primary transition" required>
            </div>
            <input type="hidden" name="year_range" id="year_range" value="{{ old('year_range') }}">
        ...
        <div class="col-span-1 md:col-span-2 flex flex-wrap items-center gap-4 mt-2">
            <div class="flex items-center gap-4 border-2 border-black p-2">
                <input type="color" name="theme_color" id="theme_color" value="{{ old('theme_color', '#ffa812') }}" class="w-8 h-8 p-0 border-0 cursor-pointer">
                <label for="theme_color" class="text-sm font-black uppercase tracking-widest cursor-pointer pr-2">Theme Color</label>
            </div>
            <div class="flex items-center gap-4">
                <input type="checkbox" name="is_present" id="is_present" value="1" {{ old('is_present') ? 'checked' : '' }} class="w-6 h-6 border-2 border-black text-black focus:ring-black rounded-none">

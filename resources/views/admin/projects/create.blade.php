...
<div class="mb-8 flex items-center gap-4">
    ...
    <div>
        <h1 class="text-4xl font-black uppercase tracking-tighter">Add Project</h1>
    </div>
</div>

<form action="{{ route('admin.projects.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8 bg-white border-2 border-black p-6 md:p-8 rounded-lg shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
    @csrf

    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
            <label class="block text-sm font-black uppercase tracking-widest mb-2">Project Title</label>
            <input type="text" name="title" value="{{ old('title') }}" class="w-full border-2 border-black p-3 focus:ring-0 focus:outline-none focus:border-primary transition" required>
        ...
        <div>
            ...
            <input type="text" name="tags" value="{{ old('tags') }}" placeholder="Redesign, Prototype, HTML" class="w-full border-2 border-black p-3 focus:ring-0 focus:outline-none focus:border-primary transition">
            @error('tags') <span class="text-red-500 text-xs font-bold">{{ $message }}</span> @enderror
        </div>

        <div>
            <label class="block text-sm font-black uppercase tracking-widest mb-2">Cover Image (Required)</label>
            <input type="file" name="image_file" accept="image/*" class="w-full border-2 border-black p-2 focus:ring-0 focus:outline-none focus:border-primary transition" required>
    ...
    <div>
        ...
        <textarea name="short_description" rows="2" class="w-full border-2 border-black p-3 focus:ring-0 focus:outline-none focus:border-primary transition" required>{{ old('short_description') }}</textarea>
    ...
    <div>
        ...
        <textarea name="full_description" rows="10" class="w-full border-2 border-black p-3 focus:ring-0 focus:outline-none focus:border-primary transition font-mono text-sm">{{ old('full_description') }}</textarea>
